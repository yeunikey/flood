import { Modal, Paper, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, TablePagination, IconButton, Button, Typography, Zoom } from "@mui/material";
...
interface Variable { id: number; name: string; }
interface DataValue { id: number; value: string; variable: Variable; }
interface GroupedData {
    group: {
        id: number; date_utc: string;
        category: { id: number; name: string };
        site: { id: number; code: string; name: string };
    };
    values: DataValue[];
}
interface PaginatedResult<T> { content: T[]; total: number; page: number; limit: number; totalPages: number; }
...
export default function FullscreenTableModal({ open, onClose, variables }: FullscreenTableModalProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const res = await api.get<ApiResponse<PaginatedResult<GroupedData>>>(`/data/category/${selectedCategory.id}/by-site/${selectedSite.code}/paginated`, {
                    ...
                    <ModalBox className="relative w-[95dvw]! h-[95dvh]! overflow-hidden flex flex-col outline-none pointer-events-auto bg-white rounded-lg shadow-xl">
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-white">
                            <div className="flex items-center gap-4">
                                ...
                                <Typography variant="body2" className="text-gray-500 bg-gray-100 px-2 py-1 rounded border border-gray-200">
                            ...
                            <Paper sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', overflow: 'hidden' }}>
                                <TableContainer sx={{ flex: 1 }}>
                                    <Table stickyHeader size="small">
                                        <TableHead>
                                            <TableRow>
                                                <TableCell align="center" sx={{ whiteSpace: 'nowrap', backgroundColor: '#f9fafb', fontWeight: 600 }}>#</TableCell>
                                                <TableCell sx={{ whiteSpace: 'nowrap', backgroundColor: '#f9fafb', fontWeight: 600 }}>Время измерения</TableCell>

                                                {variables.filter(v => !disabledVariables.includes(v.id))
                                                    .map(v => (
                                                        <TableCell key={v.id} sx={{ whiteSpace: 'nowrap', backgroundColor: '#f9fafb', fontWeight: 600 }}>
                                                            {v.name}
                                                        </TableCell>
                                                    ))}
                                            </TableRow>
                                                ...
                                                <TableRow key={group.group.id} hover>
                                                    ...
                                                    {variables.filter(v => !disabledVariables.includes(v.id))
                                                        .map(variable => {
                                                            ...
                                                            return <TableCell key={variable.id} sx={{ whiteSpace: 'nowrap' }}>
                                                                {value?.value ?? '-'}
                                                            </TableCell>;
                                                        })}
                                                </TableRow>
