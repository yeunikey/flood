...
import { Button, Paper, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, TablePagination, Zoom, Tooltip } from "@mui/material";
...
function TableInfo() {
    ...
    useEffect(() => {
        api.get<ApiResponse<Variable[]>>(`/data/category/${selectedCategory?.id}/variables`, { headers: { Authorization: `Bearer ${token}` } })
            ...
            <div className="flex flex-col h-full w-full">
                ...
                <div className="flex-1 relative min-h-0 border border-gray-200 rounded-lg overflow-hidden">
                    <div className="absolute inset-0 flex flex-col bg-white">
                        <Paper sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', boxShadow: 'none' }}>
                            <TableContainer sx={{ flex: 1, width: '100%' }}>
                                <Table stickyHeader size="small">
                                    <TableHead>
                                        <TableRow>
                                            <TableCell align="center" sx={{ whiteSpace: 'nowrap', backgroundColor: '#f5f5f5' }}>#</TableCell>
                                            <TableCell sx={{ whiteSpace: 'nowrap', backgroundColor: '#f5f5f5' }}>Время измерения</TableCell>
                                            {infoVariables.filter(v => !disabledVariables.includes(v.id))
                                                .map(v => (
                                                    <TableCell key={v.id} sx={{ whiteSpace: 'nowrap', backgroundColor: '#f5f5f5' }}>
                                                        {v.name}
                                                    </TableCell>
                                                ))}
                                        </TableRow>
