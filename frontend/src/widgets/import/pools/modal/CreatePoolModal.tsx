...
function CreatePoolModal() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post<ApiResponse<Pool>>(
        "data/pools/create",
        payload,
        {
          headers: {
      ...
      <ModalBox className="relative w-5xl! overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-6 h-124">
          ...
          <div className="flex flex-col gap-3 overflow-y-scroll h-full">
            ...
            {value === 2 && (
              <>
                {filteredHecRas.map((hecRasItem) => (
                  <label
                    key={hecRasItem.id}
                    className="flex items-center cursor-pointer"
                  >
                    <Checkbox
                      checked={selectedHecRas.includes(hecRasItem.id)}
                      onChange={() => toggleHecRas(hecRasItem.id)}
                    />
                    <span>{hecRasItem.name}</span>
                  </label>
                ))}
                {filteredHecRas.length === 0 && (
                  <Typography variant="caption" color="text.secondary">
                    Ничего не найдено
                  </Typography>
                )}
